...
public class ImageUtils {
    ...
    public static InputStream overlapImage(URL bigPicUrl, URL smallPicUrl, int xOffset, int yOffset) {
        try {
            ...
            log.error("overlap image error...，exception：" + e);
     ...
     * @return 输入流
     */
    public static InputStream overlapImage(BufferedImage big, BufferedImage small, String suffix, int xOffset,
                                           int yOffset) {
        Graphics2D g = null;
        try {
     ...
     * @param fontSize     字号大小
     */
    public static InputStream drawStringForImage(InputStream picFileInput, String content, Color contentColor,
                                                 int charNum, int fontSize) {
        Graphics2D g = null;
        try {
    ...
    public static InputStream scaleImage2Jpg(File sourceFile, double scale) throws IOException {
        return scaleImage2Jpg(Files.newInputStream(sourceFile.toPath()), scale);
    }
    ...
    public static InputStream scaleImage2Jpg(File sourceFile, int width, int height) throws IOException {
        return scaleImage2Jpg(Files.newInputStream(sourceFile.toPath()), width, height);
    }
     ...
     * @throws Exception 异常
     */
    public static InputStream regionImage(File sourceFile, int xOffset, int yOffset, int width,
     ...
     * @throws Exception 异常
     */
    public static InputStream regionImage(BufferedImage sourceImage, int xOffset, int yOffset, int width,
                                          int height) throws Exception {
        @Cleanup ByteArrayOutputStream os = new ByteArrayOutputStream();
    ...
    public static BufferedImage regionImage(URL imageUrl, int xOffset, int yOffset, int width, int height) {
        ...
        try {
            bufferedImage = Thumbnails.of(urlFileCheck(imageUrl))
                .sourceRegion(new Rectangle(xOffset, yOffset, width, height))
     ...
     *
     * @param imageUrl     源图片地址
     * @param xStartOffset 左上角x坐标偏移量
     * @param yStartOffset 左上角y坐标偏移量
     * @param xEndOffset   右下角x坐标偏移量
     * @param yEndOffset   右下角y坐标偏移量
     * @return 裁剪结果流
     * @throws Exception 异常
     */
    public static InputStream regionImageDiagonal(URL imageUrl, int xStartOffset, int yStartOffset, int xEndOffset,
                                                  int yEndOffset) throws Exception {
        @Cleanup ByteArrayOutputStream os = new ByteArrayOutputStream();
        int width = Math.abs(xEndOffset - xStartOffset);
        int height = Math.abs(yEndOffset - xEndOffset);
        Thumbnails.of(urlFileCheck(imageUrl)).sourceRegion(new Rectangle(xStartOffset, yStartOffset, width, height))
            .size(width, height).outputFormat(ImageFormatEnum.PNG.getName()).toOutputStream(os);
        return new ByteArrayInputStream(os.toByteArray());
    }
